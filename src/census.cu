...
namespace {
	...
	__global__
		void census_kernel2(int hor, int vert, SRC_T* d_source, uint64_t* d_dest, int width, int height)
	{
		const int i = threadIdx.y + blockIdx.y * blockDim.y;
		const int j = threadIdx.x + blockIdx.x * blockDim.x;
		const int offset = j + i * width;
...
namespace details {
		...
		void census(
			...
			if (depth_bits == 16) {
				census_kernel2<uint16_t> << <blocks, threads, 0, cuda_stream >> > (9, 7, (uint16_t*)d_src, d_dst, width, height);
			}
			else if(depth_bits == 8) {
				census_kernel2<uint8_t> << <blocks, threads, 0, cuda_stream >> > (9, 7, (uint8_t*)d_src, d_dst, width, height);
			}
		}
