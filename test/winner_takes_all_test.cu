...
static void test_random_left(bool subpixel, size_t padding = 0)
{
	...
	wta.enqueue(d_input.data().get(), width, height, static_cast<int>(pitch), uniqueness, subpixel, sgm::PathType::SCAN_8PATH, 0, 0);
...
static void test_random_right(size_t padding = 0)
{
	...
	sgm::WinnerTakesAll<disparity> wta;
	const auto d_input = to_device_vector(input);
	wta.enqueue(d_input.data().get(), width, height, static_cast<int>(pitch), uniqueness, false, sgm::PathType::SCAN_8PATH, 0, 0);
	cudaStreamSynchronize(0);
